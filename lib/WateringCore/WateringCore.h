...
class WateringLogic {
  public:
    ...
    bool shouldStart(int currentHour, int currentMin, int currentDayOfYear, 
      ...
      if (wateredCount >= maxCycles) return false;
      if (currentHour != targetHour || currentMin != targetMin) return false;
      if (hasWateredToday) return false;
      if (lastWateredDay == -999) return true;

      int daysDiff = currentDayOfYear - lastWateredDay;
      if (daysDiff < 0) daysDiff += 365; // 跨年处理

      if (daysDiff >= intervalDays) return true;
